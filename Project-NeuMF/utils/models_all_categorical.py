...
class GMF(nn.Module):
    def __init__(self, n_users, n_items, n_genres, n_ages, n_occupations, k) -> None:
        super(GMF, self).__init__()
        
        self.P = nn.Linear(n_users+2+n_ages+n_occupations, k, bias=False)
        self.Q = nn.Linear(n_items+n_genres, k, bias=False)
        self.h = nn.Linear(k, 5, bias=False)
    ...
    def forward(self, user_ids, item_ids, genres_one_hot, sxs_one_hot, ages_one_hot, occ_one_hot):
        ...
        data_user_ids = torch.cat([
            sparse_user_ids,
            sxs_one_hot.to_sparse(), 
            ages_one_hot.to_sparse(), 
            occ_one_hot.to_sparse(),
        ], dim=1)
        
        pu = self.P(data_user_ids)
...
class MLP(nn.Module):
    def __init__(self, n_users, n_items, n_genres, n_ages, n_occupations, k, layer_sizes) -> None:
        ...
        self.h = nn.Linear(prev_size, 5, bias=False)
...
class NeuFM(nn.Module):
    ...
    def forward(self, user_ids, item_ids, genres_one_hot, sxs_one_hot, ages_one_hot, occ_one_hot):
        Xg = self.gmf(user_ids, item_ids, genres_one_hot, sxs_one_hot, ages_one_hot, occ_one_hot)
        Xm = self.mlp(user_ids, item_ids, genres_one_hot, sxs_one_hot, ages_one_hot, occ_one_hot)
